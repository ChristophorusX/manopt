...
def _stochastic_block_model(n, p, q):
    z_sbm = np.hstack((np.ones(int(n/2)),-np.ones(int(n/2)))).reshape(n,1)
    R1_vec = aux.rounding_with_prob(np.random.random_sample(int(n/2) * int(n/2)), p)
    R3_vec = aux.rounding_with_prob(np.random.random_sample(int(n/2) * int(n/2)), p)
    ...
    for i in range(0, int(n/2)):
        for j in range(0, int(n/2)):
            if i > j:
                ...
                R1_pre[i][i] = 1
                R3_pre[i][i] = 1
    R2_vec = aux.rounding_with_prob(np.random.random_sample(int(n/2) * int(n/2)), q)
