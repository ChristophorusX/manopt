...
@function
def smallestinconvexhull(M=None,x=None,U=None,*args,**kwargs):
    varargin = smallestinconvexhull.varargin
    nargin = smallestinconvexhull.nargin
...
# [u_norm, coeffs, u] = smallestinconvexhull(M, x, U)
    ...
    N=numel(U)
# smallestinconvexhull.m:55
    G=grammatrix(M,x,U)
# smallestinconvexhull.m:56
    
    # If the optimization toolbox is not available, consider replacing with
    # CVX.
    
    opts=optimset('Display','off')
# smallestinconvexhull.m:62
    s_opt,cost_opt=quadprog(G,zeros(N,1),[],[],ones(1,N),1,zeros(N,1),ones(N,1),[],opts,nargout=2)
# smallestinconvexhull.m:64
    
    u_norm=real(sqrt(dot(2,cost_opt)))
# smallestinconvexhull.m:73
    
    coeffs=copy(s_opt)
# smallestinconvexhull.m:76
    
    if nargout >= 3:
        u=lincomb(M,x,U,coeffs)
# smallestinconvexhull.m:80
    
    
    return u_norm,coeffs,u
