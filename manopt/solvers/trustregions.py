...
# trustregions/trustregions.m

    
@function
def trustregions(problem=None,x=None,options=None,*args,**kwargs):
    varargin = trustregions.varargin
    nargin = trustregions.nargin
...
# trustregions/trustregions.m:314
...
# trustregions/trustregions.m:323
    
    # Define some strings for display
    tcg_stop_reason=cellarray(['negative curvature','exceeded trust region','reached target residual-kappa (linear)','reached target residual-theta (superlinear)','maximum inner iterations','model increased'])
# trustregions/trustregions.m:327
    # Set local defaults here
    localdefaults.verbosity = copy(2)
# trustregions/trustregions.m:335
    localdefaults.maxtime = copy(inf)
# trustregions/trustregions.m:336
    localdefaults.miniter = copy(3)
# trustregions/trustregions.m:337
    localdefaults.maxiter = copy(1000)
# trustregions/trustregions.m:338
    localdefaults.mininner = copy(1)
# trustregions/trustregions.m:339
    localdefaults.maxinner = copy(problem.M.dim())
# trustregions/trustregions.m:340
    localdefaults.tolgradnorm = copy(1e-06)
# trustregions/trustregions.m:341
    localdefaults.kappa = copy(0.1)
# trustregions/trustregions.m:342
    localdefaults.theta = copy(1.0)
# trustregions/trustregions.m:343
    localdefaults.rho_prime = copy(0.1)
# trustregions/trustregions.m:344
    localdefaults.useRand = copy(false)
# trustregions/trustregions.m:345
    localdefaults.rho_regularization = copy(1000.0)
# trustregions/trustregions.m:346
    # Merge global and local defaults, then merge w/ user options, if any.
    localdefaults=mergeOptions(getGlobalDefaults(),localdefaults)
# trustregions/trustregions.m:349
    if logical_not(exist('options','var')) or isempty(options):
        options=struct()
# trustregions/trustregions.m:351
    
    options=mergeOptions(localdefaults,options)
# trustregions/trustregions.m:353
...
# be some fraction of the provided Delta_bar.
    if logical_not(isfield(options,'Delta_bar')):
        if isfield(problem.M,'typicaldist'):
            options.Delta_bar = copy(problem.M.typicaldist())
# trustregions/trustregions.m:360
        else:
            options.Delta_bar = copy(sqrt(problem.M.dim()))
# trustregions/trustregions.m:362
    
    if logical_not(isfield(options,'Delta0')):
        options.Delta0 = copy(options.Delta_bar / 8)
# trustregions/trustregions.m:366
    
    # Check some option values
    assert_(options.rho_prime < 1 / 4,'options.rho_prime must be strictly smaller than 1/4.')
    assert_(options.Delta_bar > 0,'options.Delta_bar must be positive.')
    assert_(options.Delta0 > 0 and options.Delta0 < options.Delta_bar,'options.Delta0 must be positive and smaller than Delta_bar.')
    # It is sometimes useful to check what the actual option values are.
    if options.verbosity >= 3:
        disp(options)
    
    ticstart=tic()
# trustregions/trustregions.m:382
    # If no initial point x is given by the user, generate one at random.
    if logical_not(exist('x','var')) or isempty(x):
        x=problem.M.rand()
# trustregions/trustregions.m:386
    
    # Create a store database and get a key for the current x
    storedb=StoreDB(options.storedepth)
# trustregions/trustregions.m:390
    key=storedb.getNewKey()
# trustregions/trustregions.m:391
    ## Initializations
    
    # k counts the outer (TR) iterations. The semantic is that k counts the
# number of iterations fully executed so far.
    k=0
# trustregions/trustregions.m:397
    # Initialize solution and companion measures: f(x), fgrad(x)
    fx,fgradx=getCostGrad(problem,x,storedb,key,nargout=2)
# trustregions/trustregions.m:400
    norm_grad=problem.M.norm(x,fgradx)
# trustregions/trustregions.m:401
    # Initialize trust-region radius
    Delta=options.Delta0
# trustregions/trustregions.m:404
    # Save stats in a struct array info, and preallocate.
    if logical_not(exist('used_cauchy','var')):
        used_cauchy=matlabarray([])
# trustregions/trustregions.m:408
    
    stats=savestats(problem,x,storedb,key,options,k,fx,norm_grad,Delta,ticstart)
# trustregions/trustregions.m:410
    info[1]=stats
# trustregions/trustregions.m:411
    info[min(10000,options.maxiter + 1)].iter = copy([])
# trustregions/trustregions.m:412
    # ** Display:
    if options.verbosity == 2:
        fprintf(cat('%3s %3s      %5s                %5s     ','f: %+e   |grad|: %e\\n'),'   ','   ','     ','     ',fx,norm_grad)
    else:
        if options.verbosity > 2:
            fprintf('************************************************************************\\n')
...
# trustregions/trustregions.m:429
    consecutive_TRminus=0
# trustregions/trustregions.m:430
    # **********************
# ** Start of TR loop **
# **********************
    while true:
        ...
        ticstart=tic()
# trustregions/trustregions.m:439
        stop,reason=stoppingcriterion(problem,x,options,info,k + 1,nargout=2)
# trustregions/trustregions.m:442
    ...
    # saddle points.
        if stop == 2 and options.useRand and k < options.miniter:
            stop=0
# trustregions/trustregions.m:449
        if stop:
            if options.verbosity >= 1:
                fprintf(cat(reason,'\\n'))
            break
        if options.verbosity > 2 or options.debug > 0:
            fprintf('************************************************************************\\n')
        # *************************
    # ** Begin TR Subproblem **
    # *************************
...
# trustregions/trustregions.m:470
        else:
            # Random vector in T_x M (this has to be very small)
            eta=problem.M.lincomb(x,1e-06,problem.M.randvec(x))
# trustregions/trustregions.m:473
            while problem.M.norm(x,eta) > Delta:

                eta=problem.M.lincomb(x,sqrt(sqrt(eps)),eta)
# trustregions/trustregions.m:476

        # Solve TR subproblem approximately
        eta,Heta,numit,stop_inner=tCG(problem,x,fgradx,eta,Delta,options,storedb,key,nargout=4)
# trustregions/trustregions.m:481
        srstr=tcg_stop_reason[stop_inner]
# trustregions/trustregions.m:483
    ...
    # them have changed.
        if options.useRand:
            used_cauchy=copy(false)
# trustregions/trustregions.m:491
            Hg=getHessian(problem,x,fgradx,storedb,key)
# trustregions/trustregions.m:493
            g_Hg=problem.M.inner(x,fgradx,Hg)
# trustregions/trustregions.m:494
            if g_Hg <= 0:
                tau_c=1
# trustregions/trustregions.m:496
            else:
                tau_c=min(norm_grad ** 3 / (dot(Delta,g_Hg)),1)
# trustregions/trustregions.m:498
            # and generate the Cauchy point.
            eta_c=problem.M.lincomb(x,dot(- tau_c,Delta) / norm_grad,fgradx)
# trustregions/trustregions.m:501
            Heta_c=problem.M.lincomb(x,dot(- tau_c,Delta) / norm_grad,Hg)
# trustregions/trustregions.m:502
            # returned eta, we might as well keep the best of them.
            mdle=fx + problem.M.inner(x,fgradx,eta) + dot(0.5,problem.M.inner(x,Heta,eta))
# trustregions/trustregions.m:506
            mdlec=fx + problem.M.inner(x,fgradx,eta_c) + dot(0.5,problem.M.inner(x,Heta_c,eta_c))
# trustregions/trustregions.m:508
            if mdlec < mdle:
                eta=copy(eta_c)
# trustregions/trustregions.m:511
                Heta=copy(Heta_c)
# trustregions/trustregions.m:512
                used_cauchy=copy(true)
# trustregions/trustregions.m:513
...
# trustregions/trustregions.m:522
        if options.debug > 0:
            testangle=problem.M.inner(x,eta,fgradx) / (dot(norm_eta,norm_grad))
# trustregions/trustregions.m:525
        # Compute the tentative next iterate (the proposal)
        x_prop=problem.M.retr(x,eta)
# trustregions/trustregions.m:530
        key_prop=storedb.getNewKey()
# trustregions/trustregions.m:531
        fx_prop=getCost(problem,x_prop,storedb,key_prop)
# trustregions/trustregions.m:534
        # Check the performance of the quadratic model against the actual cost.
        rhonum=fx - fx_prop
# trustregions/trustregions.m:538
        rhoden=- problem.M.inner(x,fgradx,eta) - dot(0.5,problem.M.inner(x,eta,Heta))
# trustregions/trustregions.m:539
    ...
    # iterations if very fine convergence is demanded.
        rho_reg=dot(dot(max(1,abs(fx)),eps),options.rho_regularization)
# trustregions/trustregions.m:569
        rhonum=rhonum + rho_reg
# trustregions/trustregions.m:570
        rhoden=rhoden + rho_reg
# trustregions/trustregions.m:571
        if options.debug > 0:
...
# trustregions/trustregions.m:598
        if logical_not(model_decreased):
            srstr=matlabarray(cat(srstr,', model did not decrease'))
# trustregions/trustregions.m:601
        rho=rhonum / rhoden
# trustregions/trustregions.m:604
    ...
    # nice if we can handle them nonetheless).
        ...
        if options.debug > 0:
            m=lambda x=None,eta=None: getCost(problem,x,storedb,key) + getDirectionalDerivative(problem,x,eta,storedb,key) + dot(0.5,problem.M.inner(x,getHessian(problem,x,eta,storedb,key),eta))
# trustregions/trustregions.m:621
            zerovec=problem.M.zerovec(x)
# trustregions/trustregions.m:625
            actrho=(fx - fx_prop) / (m[x,zerovec] - m[x,eta])
# trustregions/trustregions.m:626
            fprintf('DBG:   new f(x) : %+e\\n',fx_prop)
            fprintf('DBG: actual rho : %e\\n',actrho)
            fprintf('DBG:   used rho : %e\\n',rho)
        # Choose the new TR radius based on the model performance
        trstr='   '
# trustregions/trustregions.m:633
        # then reduce the TR radius.
        if rho < 1 / 4 or logical_not(model_decreased) or isnan(rho):
            trstr='TR-'
# trustregions/trustregions.m:637
            Delta=Delta / 4
# trustregions/trustregions.m:638
            consecutive_TRplus=0
# trustregions/trustregions.m:639
            consecutive_TRminus=consecutive_TRminus + 1
# trustregions/trustregions.m:640
            if consecutive_TRminus >= 5 and options.verbosity >= 1:
                consecutive_TRminus=- inf
# trustregions/trustregions.m:642
    ...
    # initial and maximum radii.
            ...
            if rho > 3 / 4 and (stop_inner == 1 or stop_inner == 2):
                trstr='TR+'
# trustregions/trustregions.m:653
                Delta=min(dot(2,Delta),options.Delta_bar)
# trustregions/trustregions.m:654
                consecutive_TRminus=0
# trustregions/trustregions.m:655
                consecutive_TRplus=consecutive_TRplus + 1
# trustregions/trustregions.m:656
                if consecutive_TRplus >= 5 and options.verbosity >= 1:
                    consecutive_TRplus=- inf
# trustregions/trustregions.m:658
                ...
                consecutive_TRplus=0
# trustregions/trustregions.m:665
                consecutive_TRminus=0
# trustregions/trustregions.m:666
        # Choose to accept or reject the proposed step based on the model
    # performance. Note the strict inequality.
        if model_decreased and rho > options.rho_prime:
            accept=copy(true)
# trustregions/trustregions.m:672
            accstr='acc'
# trustregions/trustregions.m:673
            x=copy(x_prop)
# trustregions/trustregions.m:674
            key=copy(key_prop)
# trustregions/trustregions.m:675
            fx=copy(fx_prop)
# trustregions/trustregions.m:676
            fgradx=getGradient(problem,x,storedb,key)
# trustregions/trustregions.m:677
            norm_grad=problem.M.norm(x,fgradx)
# trustregions/trustregions.m:678
        else:
            accept=copy(false)
# trustregions/trustregions.m:680
            accstr='REJ'
# trustregions/trustregions.m:681
        # Make sure we don't use too much memory for the store database
        storedb.purge()
        k=k + 1
# trustregions/trustregions.m:689
        # Everything after this in the loop is not accounted for in the timing.
        stats=savestats(problem,x,storedb,key,options,k,fx,norm_grad,Delta,ticstart,info,rho,rhonum,rhoden,accept,numit,norm_eta,used_cauchy)
# trustregions/trustregions.m:693
        info[k + 1]=stats
# trustregions/trustregions.m:696
        # ** Display:
        if options.verbosity == 2:
            fprintf(cat('%3s %3s   k: %5d     num_inner: %5d     ','f: %+e   |grad|: %e   %s\\n'),accstr,trstr,k,numit,fx,norm_grad,srstr)
        else:
            if options.verbosity > 2:
                if options.useRand and used_cauchy:
    ...
    info=info[1:k + 1]
# trustregions/trustregions.m:726
    if (options.verbosity > 2) or (options.debug > 0):
        fprintf('************************************************************************\\n')
    
    if (options.verbosity > 0) or (options.debug > 0):
        fprintf('Total time is %f [s] (excludes statsfun)\\n',info[end()].time)
    
    # Return the best cost reached
    cost=copy(fx)
# trustregions/trustregions.m:737
...
@function
def savestats(problem=None,x=None,storedb=None,key=None,options=None,k=None,fx=None,norm_grad=None,Delta=None,ticstart=None,info=None,rho=None,rhonum=None,rhoden=None,accept=None,numit=None,norm_eta=None,used_cauchy=None,*args,**kwargs):
    varargin = savestats.varargin
    nargin = savestats.nargin

    stats.iter = copy(k)
# trustregions/trustregions.m:749
    stats.cost = copy(fx)
# trustregions/trustregions.m:750
    stats.gradnorm = copy(norm_grad)
# trustregions/trustregions.m:751
    stats.Delta = copy(Delta)
# trustregions/trustregions.m:752
    if k == 0:
        stats.time = copy(toc(ticstart))
# trustregions/trustregions.m:754
        stats.rho = copy(inf)
# trustregions/trustregions.m:755
        stats.rhonum = copy(NaN)
# trustregions/trustregions.m:756
        stats.rhoden = copy(NaN)
# trustregions/trustregions.m:757
        stats.accepted = copy(true)
# trustregions/trustregions.m:758
        stats.numinner = copy(NaN)
# trustregions/trustregions.m:759
        stats.stepsize = copy(NaN)
# trustregions/trustregions.m:760
        if options.useRand:
            stats.cauchy = copy(false)
# trustregions/trustregions.m:762
    else:
        stats.time = copy(info[k].time + toc(ticstart))
# trustregions/trustregions.m:765
        stats.rho = copy(rho)
# trustregions/trustregions.m:766
        stats.rhonum = copy(rhonum)
# trustregions/trustregions.m:767
        stats.rhoden = copy(rhoden)
# trustregions/trustregions.m:768
        stats.accepted = copy(accept)
# trustregions/trustregions.m:769
        stats.numinner = copy(numit)
# trustregions/trustregions.m:770
        stats.stepsize = copy(norm_eta)
# trustregions/trustregions.m:771
        if options.useRand:
            stats.cauchy = copy(used_cauchy)
# trustregions/trustregions.m:773
...
# trustregions/trustregions.m:780
    return stats
